...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
    ...
    // This thread is responsible for calculating C[Row][Col]
    // Needs values A[Row][k] and B[k][Col] , k = 0 ..... 
    ...
    // 1. loads tile amount of A and tile amount of B 
    // 2. gets the partial sum of length tile_width 
    // Number of such iterations required = Number of such tiles required to cover numAColumns of data
    // It will be ceil(numAColumns/TILE_WIDTH) = (((numAColumns - 1) / TILE_WIDTH) + 1)
    for (int ph = 0; ph < (((numAColumns - 1) / TILE_WIDTH) + 1); ph++) {
        // Loading tile amount of data
        // offset is ph*TILE_WIDTH
        // sA[threadIdx.y][threadIdx.x] loads the data of A[Row][threadIdx.x] with offset
        // sB[threadIdx.y][threadIdx.x] loads the data of B[threadIdx.y][Col] with offset
